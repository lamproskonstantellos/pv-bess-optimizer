...
driver value that produced it, draw a base-case dashed line and a
``Base = ...`` annotation, and fold the +/- range into the y-axis tick
labels.  Old callers that pass a frame without the driver-value
metadata fall back to the annotation-free layout.
"""
...
def test_tornado_base_annotation_present(tmp_path, monkeypatch):
    ...
    assert "Base = 15.9% IRR" in annotations
    ...
    ax = captured["fig"].axes[0]
    annotations = [t.get_text() for t in _bbox_texts(ax)]
    assert "Base = €9.0M NPV" in annotations
...
def test_tornado_backward_compat(tmp_path, monkeypatch):
    """A frame without driver-value metadata renders the v0.8.7 layout:
    no driver-value labels, no base annotation, no +/- range labels."""
    captured = _capture(monkeypatch)
    ...
    annotations = [t.get_text() for t in _bbox_texts(ax)]
    assert not any(a.startswith("Base = ") and a.endswith("IRR")
