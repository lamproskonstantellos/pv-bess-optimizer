...
def _solved_vnb_short(short_params, short_ts):
...
def test_compute_kpis_keys_are_lowercase(short_params, _solved_vnb_short):
    res, e_cap = _solved_vnb_short
    kpis = compute_kpis(res, short_params, e_cap, verify_balance=False)
    for key in kpis:
        assert key == key.lower(), f"KPI key {key!r} not lowercase"


def test_compute_kpis_contains_canonical_lowercase_keys(short_params, _solved_vnb_short):
    res, e_cap = _solved_vnb_short
    kpis = compute_kpis(res, short_params, e_cap, verify_balance=False)
    for key in (
        "profit_total_eur", "system_total_export_mwh", "bess_total_charge_mwh",
        "soc_min_pct", "pv_energy_curtailed_mwh", "e_cap_opt_mwh",
        "load_coverage_from_pv_frac",
    ):
        assert key in kpis, f"missing canonical KPI key {key!r}"


def test_no_uppercase_kpi_keys(short_params, _solved_vnb_short):
...
def test_verify_energy_balance_residuals_under_tolerance(short_params, _solved_vnb_short):
    res, _e = _solved_vnb_short
...
def test_attribute_green_discharge_adds_columns(short_params, _solved_vnb_short):
    res, _ = _solved_vnb_short
...
def test_add_economic_columns_lowercase(short_params, _solved_vnb_short):
    res, _ = _solved_vnb_short
...
def test_merchant_zeroes_load_kpis(short_params_merchant, short_ts):
    ...
    kpis = compute_kpis(res, short_params_merchant, e_cap, verify_balance=False)
...
def test_compute_monthly_kpis_lowercase(short_params, _solved_vnb_short):
    res, _ = _solved_vnb_short
