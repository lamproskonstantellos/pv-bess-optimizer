...
.. list-table::
   ...
   * - DAM price
     - 0.20 (MAPE)
     - ENTSO-E D+1 benchmark for volatile markets
   * - PV generation
     - 0.12 (RMSE)
     - NREL day-ahead PV forecast study
   * - Load
     - 0.05 (MAPE)
     - Predictable customer benchmark
...
.. list-table::
   ...
   * - ``uncertainty_load_enabled``
     - ``TRUE``
     - Apply load noise (forced FALSE in merchant mode).
   * - ``uncertainty_sigma_dam``
     - ``0.20``
     - Log-normal sigma, DAM.
...
``--compare-uncertainty-sources`` are overrides: omitted on the command
line, the workbook value applies.
...
Each ensemble produces ``uncertainty_n_seeds`` Monte Carlo
realisations.  The four DataFrames are concatenated with a
``source_set`` column and written to
``03_results.xlsx → rolling_horizon_compare_mc``.  Four new KPI keys
land on ``03_results.xlsx → financial_kpis``:
