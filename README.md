...
Mixed-integer linear programming model for PV + BESS sizing and hourly
...
└── pvbess_opt/                   # Library code
    ...
    ├── optimization.py           # Pyomo MILP, solver dispatch, 8 audit invariants
...
The `--strict` flag turns the eight dispatch invariants from warnings into
...
profit objective maximises self-consumption emergently via the load-
priority slack and produces the same dispatch as a "green" objective in
this market.  In `merchant` mode there is no load to "be green about" in
the first place.  See `docs/source/technical.documentation/objectives.rst`
